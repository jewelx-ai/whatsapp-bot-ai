-- Dashboard analytics RPC signature compatibility (2026-07-31)
-- PostgREST/Supabase may look up named RPC arguments as (p_days, p_org_id).
-- Keep the canonical (p_org_id, p_days) function and add a reversed wrapper.
...
create or replace function public.dashboard_analytics(p_days int, p_org_id uuid)
returns jsonb
language sql
stable
security definer
set search_path = public
as $$
  select public.dashboard_analytics(p_org_id, p_days);
$$;

revoke all on function public.dashboard_analytics(uuid, int) from public;
revoke all on function public.dashboard_analytics(int, uuid) from public;
grant execute on function public.dashboard_analytics(uuid, int) to service_role;
grant execute on function public.dashboard_analytics(int, uuid) to service_role;
